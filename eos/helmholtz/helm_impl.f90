...
! routine helmeos_orig as helmeos with the table read inside the routine
      ...
      subroutine read_helm_table(tab_file_name)
      include 'helm_implno.dek'
      include 'helm_table_storage.dek'

      character(512) :: tab_file_name
...
! open the file (use softlinks to input the desired table)
       ...
       open(unit=19,file=tab_file_name,status='old')
...
!      write(6,*)
      ...
      subroutine helmeos(jlo_eos, jhi_eos, eosfail)
      include 'helm_implno.dek'
...
! end of pipeline loop
      ...
      subroutine pretty_eos_out(whose, jlo_eos, jhi_eos, eosfail)
      include 'helm_implno.dek'
      include 'helm_vector_eos.dek'
